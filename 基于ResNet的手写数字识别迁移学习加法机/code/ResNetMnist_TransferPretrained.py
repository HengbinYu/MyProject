# 该文件为使用ResNet18进行手写数字加法器的模型与训练脚本代码，模式为预训练迁移学习
...
FigPath='D:\\work\\PythonProject\\DeepLearning\\Study\\手写数字识别\\06_Transfer Learning\\figs\\TransferPretrained\\'
FilePath='D:\\work\\PythonProject\\DeepLearning\\Study\\手写数字识别\\06_Transfer Learning\\results\\Result_TransferPretrained.xlsx'
...
with pd.ExcelWriter(FilePath) as writer:
    for fraction in fractions:
        ...
        net.set_filter_values(custom_resnet18)
        if use_cuda:
            net = net.cuda()
        print(use_cuda)
        # 定义损失函数，用最小均方误差来定义损失
        ...
        SaveFigPath=FigPath+'TransferPretrained_fraction_{}'.format(fraction)+'.png'
