# 该文件为使用ResNet18进行手写数字加法器的模型与训练脚本代码，模式为只固定前两层的固定值迁移学习
...
# 固定前2层的参数
FigPath='D:\\work\\PythonProject\\DeepLearning\\Study\\手写数字识别\\06_Transfer Learning\\figs\\TransferFixed_L2\\'
FilePath='D:\\work\\PythonProject\\DeepLearning\\Study\\手写数字识别\\06_Transfer Learning\\results\\Result_TransferFixed_L2.xlsx'
...
with pd.ExcelWriter(FilePath) as writer:
    for fraction in fractions:
        ...
        plt.title('TransferFixed_L2: fraction={} ; Test_right={:.2%}, Time_cost={:.2f}s'.format(fraction,test_right_ratio,time_cost))
        SaveFigPath=FigPath+'TransferFixed_L2_{}'.format(fraction)+'.png'
